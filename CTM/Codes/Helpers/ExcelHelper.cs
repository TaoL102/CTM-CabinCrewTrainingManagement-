...
namespace CTM.Codes.Helpers
{
    ...
    {
        private static CTMDbContext db = new CTMDbContext();
        ...
        public static Stream GenerateEnglishTestTemplate(Stream stream = null)
        {
            // get cabin crew name and id list
            var listCabinCrews = db.CabinCrews.Select(c => new { c.ID, Name = c.Name,c.IsResigned }).ToList();
        ...
        internal static Stream GenerateRefresherTrainingsTemplate(Stream stream = null)
        {
            // get cabin crew name and id list
            var listCabinCrews = db.CabinCrews.Select(c => new { c.ID, Name = c.Name, c.IsResigned }).ToList();
            var list = listCabinCrews.Where(c => c.IsResigned == false)
